...
class MoneroTransaction extends Transaction {
  ...
  toString() {
    ...
    return `#${this.id}: ${this.from.description || this.from} => ${this.to.description || this.to} ${this.incoming ? '+' : '-'}${this.amount} (fee: ${this.fee})`;
  }
}
...
export default class MoneroWallet extends CsWallet {
  ...
  async load() {
    ...
    try {
      ...
      this.#migrateV2();
      this.#txs = await this.#loadTxs(this.storage.get('txInfos') || []);
      for (const tx of this.#txs) {
        this.#processTx(tx);
      }
      this.storage.set('txInfos', this.#txInfos);
      // can be used for blockchain scan in future
      if (!this.storage.get('createdAt')) {
        this.storage.set('createdAt', Date.now());
      }
      this.#balance = this.#calculateBalance();
      this.storage.set('balance', this.#balance.toString());
      if (this.storage.get('lastTxInfo')) {
        try {
          await this.addTransaction(this.storage.get('lastTxInfo'));
        } catch (err) {
          if (err instanceof NotYourTransactionError
            || err instanceof errors.InternalWalletError
            || err instanceof UnknownTransactionError) {
            this.storage.delete('lastTxInfo');
          } else {
            // NetworkError and NodeError
            throw err;
          }
        }
      }
  ...
  async _loadTxs(txInfos) {
    const txInfoMap = new Map(txInfos.map((txInfo) => [txInfo.txId.toLowerCase(), txInfo]));
    const txIds = [...txInfoMap.keys()];
    const txs = [];
  ...
  #transformTx(tx) {
    return new MoneroTransaction({
      ...
      to: tx.to || tx.meta?.to || symbols.HIDDEN_ADDRESS,
  ...
  #migrateV2() {
    ...
    if (txIds && !this.storage.get('txInfos')) {
      const txInfos = txIds.map((txId) => ({ txId }));
      this.storage.set('txInfos', txInfos);
      this.storage.delete('txIds');
    }
    const lastTxId = this.storage.get('lastTxId');
    if (lastTxId && !this.storage.get('lastTxInfo')) {
      this.storage.set('lastTxInfo', { txId: lastTxId });
      this.storage.delete('lastTxId');
    }
  }

  hasTransaction(txId) {
    const id = txId.toLowerCase();
    const txInfos = this.storage.get('txInfos') || [];
    return !!txInfos.find((txInfo) => txInfo.txId === id );
  }
  ...
  async validateTransaction(txId) {
    ...
    try {
      const [tx] = await this.#loadTxs([{ txId: id }]);
      if (!tx) {
  ...
  async addTransaction(txInfo, seed) {
    if (seed) this.typeSeed(seed);
    if (typeof txInfo === 'string') txInfo = { txId: txInfo };
    const [tx] = await this.#loadTxs([txInfo]);
    if (!tx) {
      throw new UnknownTransactionError(txInfo.txId);
    }
    if (!this.#processTx(tx, seed)) {
      throw new NotYourTransactionError(txInfo.txId);
    }
    this.#txs.push(tx);
    ...
    this.#balance = this.#calculateBalance();
    this.storage.set('balance', this.#balance.toString());
    if (this.storage.get('lastTxInfo')?.txId === tx.txId) {
      this.storage.delete('lastTxInfo');
    }
    await this.storage.save();
    this.memoizeClear(this.#getUnspentsForTx);
  ...
  async createTransaction({ feeRate, address, amount, price }, seed) {
    ...
    for (const source of sources) {
      const address = wallet.getSubaddress(source.addressIndex.major, source.addressIndex.minor);
      this.#generateKeyImage(source.derivation, source.index, address);
      mixins.push({
        amount: '0',
    ...
    const { rawTx, txKey } = moneroCoreJs.createTx({
    ...
    // save tx info and key images
    const lastTxInfo = {
      txId: monerolib.tx.getTxIdFromHex(rawTx).toString('hex'),
      meta: {
        txKey,
        to: address,
      },
    };
    this.storage.set('lastTxInfo', lastTxInfo);
    this.storage.set('keyImages', this.#cachedKeyImages);
    await this.storage.save();

    const txId = await this.#api.sendTransaction(rawTx);
    // return processed tx
    await this.addTransaction({ ...lastTxInfo, txId }, seed);
    return txId;
  }
}
