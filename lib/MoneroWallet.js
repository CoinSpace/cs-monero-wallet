...
export default class MoneroWallet extends CsWallet {
  ...
  #transformTx(tx) {
    return new MoneroTransaction({
      type: MoneroTransaction.TYPE_TRANSFER,
  ...
  async validateTransaction(txId) {
    ...
    try {
      ...
      throw new UnknownTransactionError(txId, { cause: err });
    }
  }
  ...
  async _estimateMaxAmount({ feeRate, unconfirmed = false }) {
  ...
  async _selectUtxos({ feeRate, value }) {
